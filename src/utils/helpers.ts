 ...
 */
export function getServerByGroup(
  servers: Config[],
  guildId: string,
  serverId?: string,
  debug?: boolean,
  logger?: Logger
): { server: Config; error?: string } | { server: null; error: string } {
  // 如果传入了serverId，直接按ID查找
  if (serverId) {
    const server = servers.find(s => s.id === serverId)
    if (!server) return { server: null, error: `未找到ID为 ${serverId} 的服务器` }
    return { server }
  }
  ...
  const boundServers = servers.filter(s => 
    s.groups.some(g => {
      // 1. 完全相等（包括平台前缀）
      if (g === guildId) return true
 ...
 * @returns 是否允许
 */
export function isUserAllowed(userId: string, allowedList: string[]): boolean {
 ...
 * @returns 格式化后的群组ID
 */
export function formatGroupIdForBroadcast(groupId: string, defaultPlatform: string = 'onebot'): string {
  // 如果已经包含平台前缀，直接返回
  if (groupId.includes(':')) {
    return groupId
  }
