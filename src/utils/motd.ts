...
export class MOTDQuery {
   ...
   * @returns 服务器状态
   */
  async query(host: string, port: number = 25565, timeout: number = 3000): Promise<ServerStatus> {
    try {
      ...
      const response: JavaStatusResponse = await status(host, port, options)
   ...
   * 处理不同格式的MOTD描述
   */
  private extractMotd(description: any): string {
   ...
   */
  private cleanMotd(motd: string): string {
    if (!motd) return ''
   ...
   * 简化查询方法（仅检查服务器是否在线）
   */
  async isServerOnline(host: string, port: number = 25565, timeout: number = 3000): Promise<boolean> {
    try {
