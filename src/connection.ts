...
export class ServerConnection {
  ...
  private pendingRcon: Map<string, (response: any) => void> = new Map()
  ...
  connect() {
    ...
    const options: WebSocket.ClientOptions = { family: 4 }
   ...
   */
  sendRcon(command: string, timeout = 10000): Promise<any> {
   ...
   * @param text 通知文本内容
   */
  private notifyGroups(text: string) {
    if (this._destroyed) return
   ...
   */
  waitForConnect(timeout: number = 60000): Promise<void> {
    // 如果已经连接，立即成功
