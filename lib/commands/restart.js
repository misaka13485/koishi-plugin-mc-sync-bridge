...
function apply(ctx, config, connection, logger) {
    ctx.command('mc.restart', '重启MC服务器')
        ...
        if (!groupMatch)
            return '当前群未绑定此服务器';
        if (!(0, helpers_1.isUserAllowed)(session.userId, config.admins))
            return '你没有权限执行重启';
        // 检查选项冲突
        ...
        if (!options.force && !options.kill) {
            // 正常重启：60秒倒计时
            await session.send(`服务器 ${config.name} 将在60秒后正常重启`);
            connection.sendBroadcast('系统', '服务器将在60秒后重启');
            await new Promise(resolve => setTimeout(resolve, 60000));
        }
        else {
            // 强制重启/杀死：立即执行
            await session.send(`正在执行${restartType}操作: ${config.name}`);
        }
        // 再次确认连接是否仍然存在
        if (!connection) {
        ...
        try {
            ...
            // 重启命令已发送，等待自动重连
            await session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `服务器 ${config.name} ${restartType}命令已发送，等待重连...`);
        }
        catch (err) {
            logger.error(`重启失败: ${err}`);
            await session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `${restartType}失败: ${err.message}`);
        }
    });
}
