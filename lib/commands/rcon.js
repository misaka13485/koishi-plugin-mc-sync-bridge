...
function apply(ctx, config, connection, logger) {
    ...
    ctx.command('mc.rcon <command:text>', '向MC服务器发送RCON命令')
        ...
        try {
            ...
            // 6. 发送执行结果（引用原消息）
            await session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `${rconResult}`);
        }
        catch (err) {
            // 7. 处理执行错误
            if (err instanceof TimeoutError) {
                ...
                try {
                    ...
                    await session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `RCON执行失败: ${execErr.message}`);
                }
            }
            else {
                // 12. 处理其他错误
                await session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `RCON执行失败: ${err.message}`);
            }
        }
    });
}
