...
exports.formatGroupIdForBroadcast = formatGroupIdForBroadcast;
/**
 ...
 */
function getServerByGroup(servers, guildId, serverId, debug, logger) {
 ...
 * @returns 是否允许
 */
function isUserAllowed(userId, allowedList) {
 ...
 * @returns 格式化后的群组ID
 */
function formatGroupIdForBroadcast(groupId, defaultPlatform = 'onebot') {
    // 如果已经包含平台前缀，直接返回
    if (groupId.includes(':')) {
        return groupId;
    }
